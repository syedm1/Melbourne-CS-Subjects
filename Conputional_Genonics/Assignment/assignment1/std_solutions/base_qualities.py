...
def hamming_dist(read, segement):
	...
	if len(read) != len(segement):
		return (DIST_THRESHOLD + 1, mismatches)
	for i in range(len(read)):
		if read[i] == segement[i]: continue
		dist += 1
		mismatches.append(i)
		# early termination
		if dist > DIST_THRESHOLD: return (dist, mismatches)
	return (dist, mismatches)
...
def possible_aligns(read, candidates):
	...
	for candidate, index in candidates:
		dist, mismatches = hamming_dist(read, candidate)
		if dist <= DIST_THRESHOLD:
			aligns.append((dist, index, mismatches))
	return aligns
...
def find_aligns(read_k_mers, ref_k_mers, read, reference):
	...
	checked_indexes = set() # check positions in reference
...
if __name__ == '__main__':
	...
	for read in reads:
		# gather read quality
		read_scores = read.get_quality_array()
		read_quality.append(read_scores)
		...
		pos_aligns = [(d, i, m) for (d, i, m) in pos_aligns if d == pos_best[0]]
		neg_aligns = [(d, i, m) for (d, i, m) in neg_aligns if d == neg_best[0]]
		...
		# gather quality scores at mismatched positions
		reported_align = pos_best if strand == '+' else neg_best
		if strand == '+': mismatches = reported_align[2]
		else: mismatches = [len(read.sequence) - i - 1 for i in reported_align[2]]
		for mis in mismatches:
			mism_quality[mis] = mism_quality.get(mis, [])
			mism_quality[mis].append(read_scores[mis])
	...
	# plot distribution of all read qualities
	all_read_scores = [list(i) for i in zip(*read_quality)]
	plt.boxplot(all_read_scores)
	plt.savefig('all_read_score_distribution.png')
	plt.close()

	# plot distribution of mismatch positions
	mismatch_scores = []
	for i in range(len(mism_quality.keys())):
		positions = []
		if i not in mism_quality.keys():
			mismatch_scores.append((0))
		else:
			mismatch_scores.append(tuple(mism_quality[i]))
	plt.boxplot(mismatch_scores)
	plt.savefig('mismatch_score_distribution.png')
