...
U_inf  = 1 ;
panels = its_not_a_tuna() ;  % Define the panels of a tuna
n_pan  = length(panels)   ;  % Length of this tuna
...
[xp, yp] = meshgrid( -2:mesh_res:8 , -1:mesh_res:4 );
...
y_range = (-1:.25:5).'; % Range over which to seen line for flow definition
ic0  = [ -2*ones(length(y_range),1) , y_range ]; % % Initial condition matrix
...
% axis([-2 8 -1 4]);
ylabel('y (m)'); caxis([0 2]); legend('Streamlines');
